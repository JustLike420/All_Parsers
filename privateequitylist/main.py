...
class Parser:
    def __init__(self):
        self.domain = 'https://privateequitylist.com/'
        self.wb = Workbook()
    ...
    async def starter(self):
        async with aiohttp.ClientSession(
                headers=self.headers, cookies=self.cookies, connector=self.connector
        ) as session:
            tasks = []
            for i in range(1, 100):
                task = asyncio.create_task(
                    self.get_page_links(
                        f'https://privateequitylist.com/search?industry%5B%5D=Sports%2FSportsTech&page={i}',
                        session
                    )
                )
                tasks.append(task)
